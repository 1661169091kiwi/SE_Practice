...
<template>
  <div class="water-sports-data-collection">
    ...
    <div v-else class="collection-content">
      <!-- 比分控制面板 (可选，用于记录总比分) -->
      <div class="score-board" style="margin-bottom: 20px; background: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); display: flex; justify-content: space-between; align-items: center;">
        <div class="team-score" style="text-align: center;">
          <h3 style="margin: 0 0 10px; font-size: 16px;">{{ eventInfo.teamA || 'Team A' }}</h3>
          <div class="score-control" style="display: flex; align-items: center; gap: 10px; justify-content: center;">
            <button class="control-btn" @click="updateScore('teamA', -1)" style="width: 30px; height: 30px; border-radius: 50%; border: 1px solid #ddd; background: #f5f5f5; cursor: pointer;">-</button>
            <span class="score-number" style="font-size: 20px; font-weight: bold; min-width: 30px;">{{ scores.teamA }}</span>
            <button class="control-btn" @click="updateScore('teamA', 1)" style="width: 30px; height: 30px; border-radius: 50%; border: 1px solid #ddd; background: #f5f5f5; cursor: pointer;">+</button>
          </div>
        </div>
        <div class="vs-divider" style="font-weight: bold; color: #999; font-size: 18px;">VS</div>
        <div class="team-score" style="text-align: center;">
          <h3 style="margin: 0 0 10px; font-size: 16px;">{{ eventInfo.teamB || 'Team B' }}</h3>
          <div class="score-control" style="display: flex; align-items: center; gap: 10px; justify-content: center;">
            <button class="control-btn" @click="updateScore('teamB', -1)" style="width: 30px; height: 30px; border-radius: 50%; border: 1px solid #ddd; background: #f5f5f5; cursor: pointer;">-</button>
            <span class="score-number" style="font-size: 20px; font-weight: bold; min-width: 30px;">{{ scores.teamB }}</span>
            <button class="control-btn" @click="updateScore('teamB', 1)" style="width: 30px; height: 30px; border-radius: 50%; border: 1px solid #ddd; background: #f5f5f5; cursor: pointer;">+</button>
          </div>
        </div>
      </div>
